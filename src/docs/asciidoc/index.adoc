...
== Elastic Search API
=== keyword 조회 API
* 찾고자하는 검색어를 통해 음식점을 조회하는 API 입니다.

==== Query-Parameter
include::{snippets}/es/search/query-parameters.adoc[]
* 찾고자하는 검색어를 입력합니다.

==== Request
include::{snippets}/es/search/http-request.adoc[]

==== Response
include::{snippets}/es/search/http-response.adoc[]
* 상호명 -> 주소 -> 업종명 순으로 일치하는 keyword를 찾아 반환합니다.

=== Document Save API
* Entity -> Document 객체로 저장하기 위해 사용하는 API 입니다.

==== Request
include::{snippets}/es/save/http-request.adoc[]
* Entity의 저장 속도를 Document 저장 속도가 따라가지 못한 Elastic Search의 지연저장 문제로 2차 저장 작업입니다.
* 해당 API는 ADMIN 권한으로 지정되어 해당 권한 없이 실행할 수 없습니다.

==== Response
include::{snippets}/es/save/http-response.adoc[]
* 작업이 성공적으로 완료되면 'ok'를 반환합니다.